...
# PLC (Parking Lot Converter) is geometry-based rhino plug-in that allows user to run and generate structures and housing units above the any parking layouts.
...
def RunCommand( is_interactive ):
    ...
    while iteration != "No":
        ...
        if Type_Module == "Yes":
            strPromptMove = "Where do you make new modules?"
            strOptionsMove  = ["Same_Position","Next_to_existing_module"]
            Type_Move = rs.GetString(strPromptMove,strOptionsMove[1],strOptionsMove)
            
            if Type_Move == "Next_to_existing_module":
